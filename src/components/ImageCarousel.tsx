    ...
    <div>
      <Carousel setApi={setApi} opts={{ loop: true, align: 'center' }}>
        <CarouselContent className="bg-gray-400/50">
          {images.map((src, i) => (
            <CarouselItem key={src.id ?? i} className="flex items-center justify-center">
        ...
        <CarouselPrevious size="free" className="bg-gray-500/30 py-6" />
        <CarouselNext size="free" className="bg-gray-500/30 py-6" />
      </Carousel>
    </div>
  );
}
