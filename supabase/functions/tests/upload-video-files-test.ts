...
await import("../upload-video-files/index.ts");
...
Deno.test("returns 405 for non-POST methods", async () => {
  const { response, json } = await call(
    new Request("http://localhost/functions/v1/upload-video-files", {
      method: "GET",
    }),
  );
...
Deno.test("returns 400 when videos key is missing", async () => {
  const { response, json } = await call(
    new Request("http://localhost/functions/v1/upload-video-files", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  assertEquals(response.status, 400);
  assertEquals(json.error, "Body must include a videos array");
});

Deno.test("returns 400 when videos is not a non-empty array", async () => {
  const nonArray = await call(
    new Request("http://localhost/functions/v1/upload-video-files", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ videos: "not-an-array" }),
    }),
  );

  assertEquals(nonArray.response.status, 400);
  assertEquals(nonArray.json.error, "videos must be a non-empty array");
  ...
  assertEquals(empty.json.error, "videos must be a non-empty array");
});

Deno.test("returns 400 when a video item has invalid type", async () => {
  const { response, json } = await call(
    new Request("http://localhost/functions/v1/upload-video-files", {
      method: "POST",
      headers: { "content-type": "application/json" },
