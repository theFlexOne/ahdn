...
function hasResults(body: UploadImageResponse): body is { results: UploadedImageVariantsData[] } {
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const mimeTypes = [...new Set(variants.map((variant) => variant.mimeType))].sort();
