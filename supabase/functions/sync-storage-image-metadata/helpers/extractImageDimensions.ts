...
function readUint64BE(bytes: Uint8Array, offset: number): number {
  const value = (BigInt(bytes[offset]) << 56n) |
...
function extractWebpDimensions(bytes: Uint8Array): ImageDimensions {
  ...
  if (chunkType === "VP8 ") {
    ...
    const width = bytes[chunkDataOffset + 6] |
      ((bytes[chunkDataOffset + 7] & 0x3f) << 8);
    const height = bytes[chunkDataOffset + 8] |
      ((bytes[chunkDataOffset + 9] & 0x3f) << 8);
