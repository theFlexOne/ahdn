...
function getValue(source: unknown, key: string): unknown {
  if ((typeof source !== "object" && typeof source !== "function") || source === null || !(key in source)) {
    return null;
  }
...
export function ErrorBoundary() {
  ...
  const errorName = routeError
    ? "RouteErrorResponse"
    : getText(getValue(error, "name"));
  ...
  const rawError = !routeError && !(error instanceof Error) && typeof error !== "string"
    ? formatValue(error)
    : null;
    ...
    <section className="mx-auto flex min-h-screen w-full max-w-5xl flex-col justify-center gap-6 px-6 py-16 text-gray-200">
      ...
      {details.length > 0 && (
        <dl className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {details.map((detail) => (
            ...
            >
              <dt className="text-xs font-medium uppercase tracking-[0.24em] text-gray-400">
                {detail.label}
              </dt>
              <dd className="mt-2 wrap-break-word font-mono text-sm leading-6 text-gray-100">
                {detail.value}
              </dd>
            </div>
          ))}
      ...
      {routeData && (
        <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
          ...
          <pre className="mt-3 overflow-x-auto whitespace-pre-wrap wrap-break-word rounded-2xl bg-black/40 p-4 font-mono text-sm leading-6 text-red-100">
            {routeData}
          </pre>
        </div>
      )}
      ...
      {errorCause && (
        <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
          ...
          <pre className="mt-3 overflow-x-auto whitespace-pre-wrap wrap-break-word rounded-2xl bg-black/40 p-4 font-mono text-sm leading-6 text-gray-100">
            {errorCause}
          </pre>
        </div>
      )}
